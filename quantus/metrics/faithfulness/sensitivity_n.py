...
from quantus.functions.perturb_func import batch_baseline_replacement_by_indices, baseline_replacement_by_indices
...
class SensitivityN(Metric[List[float]]):
    ...
    def custom_postprocess(
        ...
        **kwargs,
        ...
        # Get pred_deltas and att_sums from result list.
        sub_results_pred_deltas: List[Any] = [r["pred_deltas"] for r in self.evaluation_scores]
        sub_results_att_sums: List[Any] = [r["att_sums"] for r in self.evaluation_scores]
        ...
        self.evaluation_scores = [
            self.similarity_func(a=sub_results_att_sums_l[k], b=sub_results_pred_deltas_l[k])
            for k in range(max_features)
        ]
