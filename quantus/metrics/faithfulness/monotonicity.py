...
@final
class BatchMonotonicity(Metric[List[float]]):
    """
    Implementation of Monotonicity metric by Arya at el., 2019.
    ...
    def evaluate_batch(
        ...
        **kwargs,
        ...
        x_baseline = np.full((batch_size, n_features), baseline_value).reshape(*x_batch.shape)
        ...
        for perturbation_step_index in range(n_perturbations):
            ...
            # Predict on perturbed input x (that was initially filled with a constant 'perturb_baseline' value).
            x_input = model.shape_input(x_baseline, x_batch.shape, channel_first=True, batched=True)
